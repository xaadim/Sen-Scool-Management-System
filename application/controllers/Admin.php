...
class Admin extends CI_Controller
{
    ...
    if ($param1 == 'import_excel')
    {
      ...
      foreach( $xlsx->rows() as $r ) 
      {
        ...
        for( $i=0; $i < $num_cols; $i++ )
        {
          ...
          else if ($i == 1)  $data['place_of_birth']    =  $r[$i];
  ...
  function student_information($class_id = '')
    {
        ...
        $page_data['page_title']    = "Fiches d'information des élèves de la classe ".
                                            $this->crud_model->get_class_name($class_id);
        $page_data['class_id']  = $class_id;
        $this->load->view('backend/index', $page_data);
    }

    function student_information_all($student_id = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
            
        $page_data['page_name']     = 'student_information_all';
        $page_data['page_title']    = 'Liste de tous les élèves';
  ...
  function attendance_selector()
  {
    ...
    /****** DAILY inOut *****************/
    function manage_inout($date='',$month='',$year='',$teacher_id='')
    {
        if($this->session->userdata('admin_login')!=1)redirect('login' , 'refresh');
        
        if($_POST)
        {
            // Loop all the teachers of $class_id
            $teachers   =   $this->db->get_where('teacher')->result_array();
            foreach ($teachers as $row)
            {
                $inout_status  =   $this->input->post('total_daily' . $row['teacher_id']);

                $this->db->where('teacher_id' , $row['teacher_id']);
                $this->db->where('date' , $this->input->post('date'));

                $this->db->update('inout' , array('total_daily' => $inout_status));
            }

            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/manage_inout/'.$date.'/'.$month.'/'.$year.'/'.$teacher_id , 'refresh');
        }
        $page_data['date']     =    $date;
        $page_data['month']    =    $month;
        $page_data['year']     =    $year;
        ...
        $page_data['page_name']  =  'manage_inout';
        $page_data['page_title'] =  'Gestion des pointages horaires';
        $this->load->view('backend/index', $page_data);
    }
    function inout_selector()
    {
        redirect(base_url() . 'index.php?admin/manage_inout/'.$this->input->post('date').'/'.
                    $this->input->post('month').'/'.
                        $this->input->post('year').'/'.
                            $this->input->post('teacher_id') , 'refresh');
    }
    ...
    function discharge()
    {
        ...
        $page_data['page_name']     = 'discharge';
        $page_data['page_title']    = get_phrase('discharge_model');
        $this->load->view('backend/index', $page_data);
    }

    function bank_transfert($param1 = '' , $param2 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
        if ($param1 == 'create') {
            ...
            $data['date']               =   $this->input->post('date');
            $data['current_money']               =   $this->input->post('current_money');
            $this->db->insert('bank_transfert' , $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/bank_transfert', 'refresh');
        }
        ...
        if ($param1 == 'edit') {
            ...
            $data['out']         =   $this->input->post('out');
            $data['date']              =   $this->input->post('date');
            $this->db->where('bank_transfert_id' , $param2);
            $this->db->update('bank_transfert' , $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/bank_transfert', 'refresh');
        }
        ...
        if ($param1 == 'delete') {
            $this->db->where('bank_transfert_id' , $param2);
            $this->db->delete('bank_transfert');
            $this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
            redirect(base_url() . 'index.php?admin/bank_transfert', 'refresh');
        }

        $page_data['page_name']  = 'bank_transfert';
        $page_data['page_title'] = get_phrase('bank_transfert');
        $this->load->view('backend/index', $page_data); 
    }

    function origin_money($param1 = '' , $param2 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
        if ($param1 == 'create') {
            $data['origin_money']   =   $this->input->post('origin_money');
            $data['date']   =   $this->input->post('date');
            $this->db->insert('origin_money' , $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/bank_transfert');
        }
        if ($param1 == 'edit') {
            $data['origin_money']   =   $this->input->post('origin_money');
            $data['date']   =   $this->input->post('date');
            $this->db->where('origin_money_id' , $param2);
            $this->db->update('origin_money' , $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/bank_transfert');
        }
        if ($param1 == 'delete') {
            $this->db->where('origin_money_id' , $param2);
            $this->db->delete('origin_money');
            $this->db->delete('date');
            $this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
            redirect(base_url() . 'index.php?admin/bank_transfert');
        }
        ...
        $page_data['page_name']  = 'bank_transfert';
        $page_data['page_title'] = 'Fond de caisse';
        $this->load->view('backend/index', $page_data);
    }
    ...
    function transport_information($transport_id = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
            
        $page_data['page_name']     = 'transport_information';
        $page_data['page_title']    = 'Liste des élèves dans ce transport ';
        $page_data['transport_id']  = $transport_id;
        $this->load->view('backend/index', $page_data);
    }

    /**********MANAGE UNIFORMES********************/
    function uniformes($param1 = '', $param2 = '', $param3 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
        if ($param1 == 'create') {
            ...
            $data['price']                   = $this->input->post('price');
            $data['uniformes_category_id']   = $this->input->post('uniformes_category_id');
            $this->db->insert('uniformes', $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/uniformes', 'refresh');
        }
        if ($param1 == 'do_update') {
            $data['number']                     = $this->input->post('number');
            $data['price']                      = $this->input->post('price');
            ...
            $this->db->where('uniformes_id', $param2);
            $this->db->update('uniformes', $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/uniformes', 'refresh');
        } else if ($param1 == 'edit') {
            $page_data['edit_data'] = $this->db->get_where('uniformes', array(
                'uniformes_id' => $param2
            ))->result_array();
        }
        if ($param1 == 'delete') {
            $this->db->where('uniformes_id', $param2);
            $this->db->delete('uniformes');
            $this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
            redirect(base_url() . 'index.php?admin/uniformes', 'refresh');
        }
        $page_data['uniformes'] = $this->db->get('uniformes')->result_array();
        $page_data['page_name']  = 'uniformes';
        $page_data['page_title'] = get_phrase('manage_uniformes_title');
        $this->load->view('backend/index', $page_data);
        
    }

    function uniformes_information($transport_id = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
            
        $page_data['page_name']     = 'uniformes_information';
        $page_data['page_title']    = get_phrase('manage_uniformes');
        $page_data['transport_id']  = $transport_id;
        $this->load->view('backend/index', $page_data);
    }

    function uniformes_category($param1 = '' , $param2 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
        if ($param1 == 'create') {
            $data['name']   =   $this->input->post('name');
            $this->db->insert('uniformes_category' , $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/uniformes_category');
        }
        if ($param1 == 'edit') {
            $data['name']   =   $this->input->post('name');
            $this->db->where('uniformes_category_id' , $param2);
            $this->db->update('uniformes_category' , $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/uniformes_category');
        }
        if ($param1 == 'delete') {
            $this->db->where('uniformes_category_id' , $param2);
            $this->db->delete('uniformes_category');
            $this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
            redirect(base_url() . 'index.php?admin/uniformes_category');
        }

        $page_data['page_name']  = 'uniformes_category';
        $page_data['page_title'] = get_phrase('uniformes_category');
        $this->load->view('backend/index', $page_data);
    }
    ...
    function manage_admin($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        $page_data['page_title'] = 'Gestion des administrateurs';
        $this->load->view('backend/index', $page_data);
    }
